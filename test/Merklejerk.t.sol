...
contract FastLZ {
    ...
    function __decompressAndCall(
        bytes memory compressedCreationCode,
        bytes memory data
    ) external {
        bytes memory decompressedCreationCode =
            LibZip.flzDecompress(compressedCreationCode);
        ...
        assembly {
            temporaryAddr :=
                create(
                    0,
                    add(decompressedCreationCode, 0x20),
                    mload(decompressedCreationCode)
                )
        }
    ...
    function decompressAndCall(
        bytes memory compressedCreationCode,
        bytes memory data
    ) external returns (bytes memory) {
        (bool success, bytes memory returnData) = address(this).call(
            abi.encodeCall(
                this.__decompressAndCall, (compressedCreationCode, data)
            )
        );
...
contract ContractCompressionTest is Test {
    ...
    function testRoundtrip() public {
        bytes memory compressedCreationCode =
            flz.compress(type(Foo).creationCode);

        assertEq(
            abi.decode(
                flz.decompressAndCall(
                    compressedCreationCode, abi.encodeCall(Foo.add, (400, 20))
                ),
                (uint256)
            ),
            420
